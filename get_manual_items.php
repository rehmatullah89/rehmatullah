<?php
	...
	if(!$db) {
		die('Unable to select database');
			...
			echo '<script type="text/javascript"> alert("Customer Name/Phone Must not be Empty!"); </script>';
			exit();
			}
			
 $q2="SELECT item_name,unit_price,cotton_quantity,cotton_boxses,per_box_items,series_name,item_image,remaining_box_qty,remaining_item_qty FROM products where series_name='$s_name' AND (cotton_quantity>0 OR remaining_box_qty>0 OR remaining_item_qty>0)";
...
while($j < 2){		
$j++;
echo "<td><table class='imagetable' align='center'><tr><th>Serial#</th><th>Item Model</th><th>Box Qty</th><th>Items</th></tr>";
